...
class SplashVC: UIViewController {
    ...
    // MARK: Properties
    // TODO: fill each game
    // TODO: split tutorial to each page? If so, remove button from menu, and remove text that mentions that button.
    let aString = """
Welcome.
...
Feel free to skip it for now to jump to the minigames, by tapping on the X on the top right of this page.
...
MINIGAMES TO EXPERIENCE THEM YOURSELF

You can try the app's minigames in order to see if your results match the app's results, so you can sharpen
...
So when asking the Thinker "ignoring any remainer, how many times does 9 fit into the result?", we are \
...
(or 1 more, depending on whether the total amount of numbres is odd or even).
...
(Oh no! Confusing... Not really. Now you know it's higher than 250, and loewr than 500)
...
You can try it a few times in the minigame itself, and that should suffice to understand how to do it to others.
The pattern essentially is as follows:

- Thinker thinks of a number which is equal to 10 or higher
- Thinker adds all the digits of te number together (for example, if number was 24, you get 2+4=6)
...
BONUS FUN FACT: if, when adding up the digits of the original number, the result is more than one digit, Thinker can choose (and doesn't even have to tell you which option they choose, trick will still work) whether to subtract the initial sum of their original number, from their original number, or whether to add up the digits of the new result, and subtract the second new result, from the orignal number.
